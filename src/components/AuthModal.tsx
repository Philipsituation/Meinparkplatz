...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleAuthSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      onLoginSuccess({
        ...
        email: email.trim(),
        isEmailVerified: false, // Erst nach Verifizierung true
  ...
  return (
    <div className="w-full">
      ...
      {/* Header */}
      <div className="bg-[#22262d] text-white p-4 flex items-center justify-between rounded-t-2xl">
      ...
      {showVerificationBanner && (
        <div className="m-4 p-3 bg-amber-50 border border-amber-300 rounded-xl text-xs text-amber-900 space-y-1">
          ...
          <p className="text-[11px] text-amber-800">
            Bitte überprüfe deinen Posteingang für <strong>{email}</strong> und klicke auf den Bestätigungslink, um dein Konto vollständig freizuschalten.
          </p>
        </div>
      )}
